...
선형 검색은 **배열의 첫 요소부터 마지막 요소까지 순서대로 비교하면서 찾는 값이 있는지 확인하는 탐색 알고리즘**입니다.  
정렬되지 않은 배열에서도 사용할 수 있으며 구현이 매우 단순합니다.

---

## ✔ 작동 원리

1. 배열의 **첫 번째 요소부터 차례대로** 확인한다.  
2. 현재 값이 **찾는 값과 같으면 즉시 종료**하고 인덱스를 반환한다.  
3. 다르면 다음 요소로 넘어간다.  
4. 배열 끝까지 확인했는데 없으면 **검색 실패(-1)** 를 반환한다.

---

## ✔ 예시

배열: `[10, 23, 5, 7, 9]`, 찾는 값: `7`
...
## 📝 3줄 요약
- 처음부터 순서대로 비교하며 값을 찾는 방식이다.  
- 정렬이 필요 없고 구현이 간단하다.  
- 시간 복잡도는 O(n)으로 효율은 낮다.


# 🔎 이진 검색(Binary Search)

이진 검색은 **정렬된 배열을 절반씩 나누어 원하는 값을 빠르게 찾는 탐색 알고리즘**입니다.  
선형 검색보다 훨씬 빠르지만, **배열이 반드시 정렬되어 있어야** 합니다.

---

## ✔ 작동 원리

1. 배열의 **중간 위치(mid)** 를 계산한다.  
2. 중간 값이 **찾는 값과 같으면 종료**한다.  
3. 찾는 값이 중간 값보다 **작으면 왼쪽 구간**,  
   **크면 오른쪽 구간**만 다시 탐색한다.  
4. 구간을 계속 절반으로 줄여가며 반복한다.  
5. 찾는 값이 없으면 **검색 실패(-1)** 를 반환한다.

---

## ✔ 예시

배열(정렬됨): `[2, 3, 5, 7, 9, 12, 15]`  
찾는 값: `7`
...
## 📝 3줄 요약
- **정렬된 배열에서** 중앙을 기준으로 탐색 범위를 반으로 줄여 나가는 검색 방법이다.  
- 시간 복잡도는 **O(log n)** 으로 매우 빠르다.  
